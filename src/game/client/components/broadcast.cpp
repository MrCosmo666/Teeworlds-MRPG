...
{
	if(!g_Config.m_ClShowServerBroadcast || m_MuteServerBroadcast || !m_ServerBroadcastCursor.Rendered())
		return;
...
{
	if(!g_Config.m_ClShowServerBroadcast || m_MuteServerBroadcast)
		return;
	...
	for(int i = 0; i < m_NumSegments; i++)
	{
		if(g_Config.m_ClColoredBroadcast)
		{
