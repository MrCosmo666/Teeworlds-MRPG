...
{
	if(NetworkClipped(SnappingClient) || !m_pPlayer->IsVisibleForClientID(SnappingClient))
		return;
...
bool CCharacter::StartConversation(CPlayer *pTarget)
{
	...
	if (!pTargetBot || pTargetBot->GetBotType() == BotsTypes::TYPE_BOT_MOB || !pTargetBot->IsVisibleForClientID(m_pPlayer->GetCID()))
		return false;
	return true;
}
