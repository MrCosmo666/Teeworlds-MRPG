...
public:
	...
	int Init(const char *pApplicationName, int StorageType, int NumArgs, const char **ppArguments)
	{
		...
		if(StorageType != STORAGETYPE_BASIC)
		{
			if(m_NumPaths && (!m_aaStoragePaths[TYPE_SAVE][0] || !fs_makedir_recursive(m_aaStoragePaths[TYPE_SAVE])))
			{
				...
				fs_makedir(GetPath(TYPE_SAVE, "demos/mrpg", aPath, sizeof(aPath)));
		...
		else if(!str_comp(pPath, "$CURRENTDIR"))
		{
			if(m_aCurrentDir[0])
			{
				if (!IsDuplicatePath(m_aCurrentDir))
				{
					...
					dbg_msg("storage", "added path '$DATADIR' ('%s')", m_aCurrentDir);
				}
				else
					dbg_msg("storage", "skipping duplicate path '$DATADIR' ('%s')", m_aCurrentDir);
			}
		}
		else if(!str_comp(pPath, "$APPDIR"))
		{
			if(m_aAppDir[0])
			{
				if (!IsDuplicatePath(m_aAppDir))
				{
					...
					dbg_msg("storage", "added path '$DATADIR' ('%s')", m_aAppDir);
				}
				else
					dbg_msg("storage", "skipping duplicate path '$DATADIR' ('%s')", m_aAppDir);
			}
		}
		else
		{
	...
	static IStorageEngine *CreateTest()
	{
		CStorage *p = new CStorage();
		if(!p)
		{
