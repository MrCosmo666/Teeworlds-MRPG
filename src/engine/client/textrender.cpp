...
void CTextRender::LoadFonts(IStorageEngine *pStorage, IConsole *pConsole)
{
	...
	if(rFiles.type == json_array)
	{
		for(unsigned i = 0; i < rFiles.u.array.length && i < MAX_FACES; ++i)
		{
			...
			IOHANDLE File = pStorage->OpenFile(aFontName, IOFLAG_READ, IStorageEngine::TYPE_ALL, aFilename, sizeof(aFilename));
			if(File)
			{
